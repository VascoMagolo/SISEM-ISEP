...
void can_interrupt();
void can_send(const CAN_NODE_t* can_node, uint16_t can_id, uint8_t* data, uint8_t length);
...
void can_send_gps(const CAN_NODE_t* can_node, uint16_t can_id, float lat, float lon);
